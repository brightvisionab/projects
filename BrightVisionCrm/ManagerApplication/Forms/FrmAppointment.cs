...
namespace ManagerApplication.Forms {
    public partial class FrmAppointment : XtraForm {
        ...
        private bool ValidRequiredFields()
        {
            ...
            {
                MessageBox.Show("Please specify a subject.", "Scheduling", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            else if (Convert.ToInt32(resourcesEditResource.EditValue) < 1 || resourcesEditResource.Text.Length < 1)
            {
                MessageBox.Show("Please select a resource.", "Scheduling", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            ...
            {
                MessageBox.Show("Start Time is not valid.", "Scheduling", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            else if (!IsValidSqlDateTimeNative(end))
            {
                MessageBox.Show("End Time is not valid.", "Scheduling", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            return true;
        }
        ...
        private void btnOk_Click(object sender, EventArgs e) 
        {
            ...
            if (resourcesEditResource.ResourceId == null) {
                this.DialogResult = System.Windows.Forms.DialogResult.None;
                MessageBox.Show("Please specify a resource for this schedule.",
                    "Validation", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                return;
            }
            if (!IsIntervalValid()) {
                this.DialogResult = System.Windows.Forms.DialogResult.None;
                MessageBox.Show("End time should be greater than start time.",
                    "Invalid Time Schedule", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            if (!controller.IsConflictResolved()) {
                this.DialogResult = System.Windows.Forms.DialogResult.None;
                MessageBox.Show("The current schedule conflicted with the schedule of selected resource. Please select other schedule or resource.",
                    "Conflict Schedule", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
