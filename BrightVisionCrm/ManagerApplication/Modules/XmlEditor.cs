...
public partial class XmlEditor : UserControl
{
    ...
    private void contextMenuStrip1_Opening(object sender, CancelEventArgs e)
    {
        ...
        if (this.xmlTextBox.SelectedText.Length == 0)
        {
            contextMenuStrip1.Items["cmsCut"].Enabled = false;
            contextMenuStrip1.Items["cmsCopy"].Enabled = false;
            contextMenuStrip1.Items["cmsDelete"].Enabled = false;
        }
        else
        {
            contextMenuStrip1.Items["cmsCut"].Enabled = true;
            contextMenuStrip1.Items["cmsCopy"].Enabled = true;
            contextMenuStrip1.Items["cmsDelete"].Enabled = true;
        }

        // Disable Paste if Clipboard does not contains text
        if (Clipboard.ContainsText())
        {
            contextMenuStrip1.Items["cmsPaste"].Enabled = true;
        }
        else
        {
            contextMenuStrip1.Items["cmsPaste"].Enabled = false;
        }

        // Disable Select All if TextBox is blank
        if (this.xmlTextBox.Text.Length == 0)
        {
            contextMenuStrip1.Items["cmsSelectAll"].Enabled = false;
        }
        else
        {
            contextMenuStrip1.Items["cmsSelectAll"].Enabled = true;
        }
    }
    ...
    {
        this.xmlTextBox.Copy();
    }

    private void cmsPaste_Click(object sender, EventArgs e)
    {
        this.xmlTextBox.Paste();
    }

    private void cmsDelete_Click(object sender, EventArgs e)
    {
        this.xmlTextBox.SelectedText = "";
    }

    private void cmsSelectAll_Click(object sender, EventArgs e)
    {
        this.xmlTextBox.SelectAll();
    }

    private void xmlTextBox_MouseDown(object sender, MouseEventArgs e)
    {
        if (e.Button == System.Windows.Forms.MouseButtons.Right)
        {
            Point endPoint = ((Control)sender).PointToScreen(new Point(e.X, e.Y));
            contextMenuStrip1.Show(endPoint);
        }
    }
