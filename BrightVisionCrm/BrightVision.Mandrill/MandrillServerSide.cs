...
{
    public class MandrillServerSide
    {
        private string _key = "";
        ...
        public bool Send(ref string return_message)
        {
            bool bolResult = true;
            string _id = "";
            return_message = "";
            try
            {
                ...
                for (int i = 0; i < arrJson.Length; i++)
                {
                    ...
                    {
                        failedEmail += (string)o["email"] + "<br/>";
                        return_message = status;
                    }
                    else if (status.ToLower() == "sent")
                    {
                        okEmail += (string)o["email"] + "<br/>";
                        return_message = status;
                    }
                    else if (status.ToLower() == "queued")
                    {
                        queued += (string)o["email"] + "<br/>";
                        return_message = status;
                    }
                    else
                    {
                ...
                if (failedEmail != "")
                {
                    //BrightVision.Common.UI.NotificationDialog.Error("Failed", "Failed send email to following:<br/>" + failedEmail);
                    return_message = "Failed send email to following:<br/>" + failedEmail;
                }

                if (others != "")
                {
                    //BrightVision.Common.UI.NotificationDialog.Error("Error", others);
                    return_message = others;
                }

                if (queued != "")
                {
                    //BrightVision.Common.UI.NotificationDialog.Information("Success", "Email on queued:<br/>" + queued);
                    return_message = "Email on queued:<br/>" + queued;
                }

                if (okEmail != "")
                {
                    //BrightVision.Common.UI.NotificationDialog.Information("Success", "Successfully send email to following:<br/>" + okEmail);
                    return_message = "Successfully send email to following:<br/>" + okEmail;
                }
            }
            catch (Exception e)
            {
                //BrightVision.Common.UI.NotificationDialog.Error("Error", "An error has encountered when trying to send email.<br/>Please contact system administrator.");
                bolResult = false;
